...
public class Console { //persons + items
    ...
    public static void insert(SortedMap persons, String name, Palace palace) {
//        Humanoid chel = new Humanoid(name, palace);
        List<Predmet> items = new LinkedList<>();
        List<Predmet> items1 = new LinkedList<>();
        Predmet.Butilka les = new Predmet.Butilka("Шишкин лес");
        Predmet.Shlyapa mujik = new Predmet.Shlyapa("А она ему как раз");
        items.add(mujik);
        items.add(les);
        SortedMap<Humanoid, List<Predmet>> persons1 = new TreeMap<>();

        persons1.put(new Humanoid(name, palace), items);
        persons.put(persons1.firstKey(),persons1.get(persons1.firstKey()));
        System.out.println(persons1.firstKey().getName());
        System.out.println("test" + persons1);
        persons1.clear();
        System.out.println(persons1 + " clear");

    }

    public static void reader(SortedMap map, String str) { // сделать отлово arrayOutOfBounceException'ов
        int comand = 0;
        ...
        if (str.contains("exit")) comand = comand * 10 + 8;
        if (str.contains("help")) comand = comand * 10 + 9;
...
//            String a11 = a1[0];
                ...
                for (Object key : map1.keySet()) {
                    map.put(key.toString(), map1.get(key).toString());
                }
                break;
            case 2:
                String[] a2 = str.split("remove");
                map.remove(a2[1]);
                break;
            case 3:
                ...
                break;
            case 4:
                String[] a4 = str.split("add_if_max");
                if (a4[1].contains(",")) {
...
//                    if (!(a41[1].hashCode() > value.hashCode())) b4 = false;
//                    }
//                if (b4) map.put(a41[0], a41[1]);
//            }
//            else System.out.println("Введите значения ещё раз, через запятую");
...
//            str.replace("remove_lower", "");
                ...
                SortedMap<Object, Object> map7 = new TreeMap<>();
                System.out.println("hashCode element: " + element);
                for (Object key : map.keySet()) {
//                System.out.println("Key: " + key + "; his element hashCode: " + map.get(key).hashCode());
                ...
                map.clear();
                for (Object key : map7.keySet()) {
                    map.put(key.toString(), map7.get(key).toString());
                }
                map.remove(" ");
                break;
            case 8:
